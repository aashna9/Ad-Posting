...
$cresult = mysql_query($cquery,$link) or die ("cquery err");
echo 
"<div class='container'>
<table>";
while($category = mysql_fetch_array($cresult))
{
	$squery = "select * from sub_categories where c_id = '$category[c_id]'";
    $sresult = mysql_query($squery,$link) or die ("squery err");
	echo "<tr><td><div class='$category[c_id]' onMouseOver='subcat($category[c_id])'> $category[c_name] </div></td>";
	echo "<td><ul><div class='subcategory'>";
	while($sub_category = mysql_fetch_array($sresult))
	{
		echo "<li><a href='vehicle1.php?cid=$category[c_id]&&sid=$sub_category[s_id]'>$sub_category[s_name]</a></li>";
	}
    echo "</div></ul></td></tr>";
}
echo
