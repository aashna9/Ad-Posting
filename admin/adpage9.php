...
include("dbconnect.php");
 $cidquery = "select c_id from ad_details where ad_no='$adno'";
    $cidresult = mysql_query($cidquery) or die ("cidquery err");
    $cidlist = mysql_fetch_array($cidresult);
    $cid = $cidlist[0];
 $sidquery = "select s_id from ad_details where ad_no='$adno'";
    $sidresult = mysql_query($sidquery) or die ("sidquery err");
...
$adtable = "select * from $table t where t.ad_no='$adno'";
    $tableresult = mysql_query($adtable) or die ("adquerry err");
...
<h2>$adrow[ad_title]</h2>
  ...
  <div class='two_third lastcolumn'>
   ...
   while( $rrow = mysql_fetch_field($tableresult))
	 {
		   $fname = $rrow->name;
		   if($frow[$rrow->name]!='N.A.')
		echo "<p> $fname:  $frow[$fname]</p>";
	  }
echo "
...
<p class='portfolio'><a href='#'>Project Details</a> | <a href='#'>Visit Website</a></p>
  ...
  <section class="comments">
   ...
   <div id='comments'> 
	<!--related-ad column-->
	<h2>RELATED ADS</h2>";
 $caddetails = "SELECT * FROM ad_details WHERE s_id ='$sid' AND ad_no <> '$adno'";
